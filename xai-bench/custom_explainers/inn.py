    ...
    def predict(
        ...
        if self.interpretable and return_weights:
            ...
            if self.mode == 'classification':
                ...
                correct_test_examples = np.array(correct_test_examples, dtype=int)

            test_examples = X_test.detach().to('cpu').numpy()
            
            if len(correct_test_examples) > 0:
                correct_test_examples = test_examples[correct_test_examples]
            else:
                correct_test_examples = np.array([])  # Nếu không có ví dụ nào đúng, trả về mảng rỗng

            if correct_test_examples.size > 0 and weights.size > 0:
